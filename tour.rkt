...
;
; Server state:
; #( destination scene cmd-num room-switch-time pic-visible )
;
...
(define (ss-transition-time) (vector-ref *server-state* 3))
(define (ss-pic-visible) (vector-ref *server-state* 4))
...
(define (ss-transition-time! x)     (vector-set! *server-state* 3 x))
(define (ss-pic-visible! x)  (vector-set! *server-state* 4 x))


; initial server state:
(set-server-state! (vector 0 0 0 0.0 ""))
...
; launch-parms is an arbitrary s-expression that was passed to the +link
; function by another script.
;
; You can call additional + functions to add to the list that
...
;    "https://s3.amazonaws.com/o.oculuscdn.com/netasset/matt_test/New7Wonders/tajmahal_4.jpg"
...
(define (set-scene! dest scene)
  ...
  (ss-destination! dest)
  (ss-scene! scene)
  (ss-transition-time! *script-seconds*))
...
(define (time-sound time wav . options)
  (define volume (if (> (length options) 0)
                     (list-ref options 0)
                     1.0))
  (when (crossed-seconds (+ time (ss-transition-time)))
    (if (> (length options) 1)
        (+sound wav
...
(define (background-sound wav . options)
  (define volume (if (> (length options) 0)
                     (list-ref options 0)
                     1.0))
  (when (crossed-seconds (+ 0.01 (ss-transition-time)))
...
(define (swipe-anim first-time)
  ...
  (define anim-time 3.0)
    (when (and (> (- *script-seconds* (ss-transition-time) ) first-time) (not (= (ss-destination) dest-airport)))
      (let* ( (cycle (/ (- *script-seconds* (ss-transition-time) first-time) repeat-time))
              (prev-cycle (/ (- *prev-script-seconds* (ss-transition-time) first-time) repeat-time))
      ...
      (when (< 0.0 anim-frac 1.0)
        (let ((anim (* 0.1 (- *script-seconds* (floor *script-seconds*))))
              ...
              (+quad "https://s3.amazonaws.com/o.oculuscdn.com/netasset/matt_test/2d/demo_headset.png"
                     (mat4-compose
              ...
              (+quad "https://s3.amazonaws.com/o.oculuscdn.com/netasset/matt_test/2d/SwipeSuggestionArrowRight.png"
                     (mat4-compose
...
(define (tic)
  ...
  (when (pressed-back)
    (client-command! 'destination 0)) ; for demo, don't exit app at all
...
;        (client-command! 'destination 0)))
  ...
  (when (not (and (= (ss-destination) *current-destination*)
