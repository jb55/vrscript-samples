...
(define (client-command! cmd parm)
  (set-local-client-state! (vector (+ 1 (ss-cmd-num)) cmd parm)))

; initial client state:
(set-local-client-state! (vector 0 1 #f #f))
...
(define (set-position)
  (cond
    ((= *local-client-id* (seat-get 0)) (+set-position (vec3 0.0 0.0 0.0) 0.0))
    ((= *local-client-id* (seat-get 1)) (+set-position (vec3 -0.5 0.0 0.0) 0.0))
    ((= *local-client-id* (seat-get 2)) (+set-position (vec3 0.5 0.0 0.0) 0.0))
...
(define (pic-button-yaw-pitch yaw-degrees pitch-degrees pic-def)
  ...
  (define vec-right (mat4-transform-dir mat-yaw (vec3 1.0 0.0 0.0)))
